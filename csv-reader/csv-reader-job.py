...
CSV_READER_JOB_NAME = os.getenv("CSV_READER_JOB_NAME")

# setup da aplicação Spark
spark = (
    SparkSession.builder.appName(f"{CSV_READER_JOB_NAME}")
    .config("spark.sql.warehouse.dir", abspath("spark-warehouse"))
    .config("fs.s3a.endpoint", f"{S3A_ENDPOINT}")
    .config("fs.s3a.access.key", f"{S3A_ACCESS_KEY}")
    .config("fs.s3a.secret.key", f"{S3A_SECRET_KEY}")
...
df.write.format("parquet").mode("overwrite").save(
    "s3a://processing/df-parquet-file.parquet"
)
...
df_result.write.format("parquet").mode("overwrite").save(
    "s3a://curated/df-result-file.parquet"
)
